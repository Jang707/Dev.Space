...
def send_sensor_data():
    ...
    data = {
        'temperature': f"Temperature: {temperature}",  # prefix 제거
        'humidity': f"Humidity: {humidity}",          # prefix 제거
        'light_value': f"Light: {light_value}",      # prefix 제거
        'gas_status': f"Gas Status: {gas_status}"    # prefix 제거
    }
    return data
...
async def read_sensors():
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            # Gas 상태가 변경되었을 때 TCP로 전송 (prefix 제거)
            if prev_gas_status != gas_status:
                tcp_client.sendmsg({'message': f'Gas Status Changed to: {gas_status}'})
        ...
        async with session.post(f'{arduino_base_url}/set_rgb', json=rgb_command) as response:
            if response.status == 200:
                ...
                if result.get('success'):
                    ...
                    # RGB 값이 변경될 때 TCP로 전송 (prefix 제거)
                    tcp_client.sendmsg({'message': f'RGB Color Changed to: {rgb_color}'})
